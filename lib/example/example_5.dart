...
class Example5 extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    return Scaffold(
      appBar: AppBar(title: Text('Change data in ListView')),
      body: Consumer(
            ...
            itemBuilder: (context, index) {
              ...
              return ListTile(
                title: GestureDetector(
                  onTap: () async {
                    final updatePerson = await createOrUpdatePersionDialoge(
                      context,
                      person,
                    );
      ...
      floatingActionButton: FloatingActionButton(
        onPressed: () async {
          final person = await createOrUpdatePersionDialoge(context);
          if (person != null) {
  ...
  Future<Person?> createOrUpdatePersionDialoge(
    ...
    return showDialog(
      ...
      builder: (context) {
        return AlertDialog(
          title: const Text('Create a text'),
          content: Column(
...
class Person {
  ...
  Person({required this.name, required this.age, String? uid})
    : uuid = uid ?? const Uuid().v4();

  Person updated([String? name, int? age]) =>
      Person(name: name ?? this.name, age: age ?? this.age, uid: uuid);
